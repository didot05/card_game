...
public class BoardCSVConverter {
    private static ArrayList<String> boardCSVData;
    ...
    public void setBoardData() {
        for (int i = 0; i < boardCSVData.size(); i++) {
            if (i >= 4 && i <= 43) {
                ...
                if (temp.length == 6) {
                    squareData.add(new Square(Integer.parseInt(temp[0]), temp[1], temp[3].toLowerCase(), temp[4], canBeBought, 0, null));
                } else if (temp.length == 9) {
                    squareData.add(new Square(Integer.parseInt(temp[0]), temp[1], temp[2], temp[3].toLowerCase(), canBeBought,
                            Integer.parseInt(temp[7]), new Property(temp[3].toLowerCase())));
                    ...
                    squareData.add(new Square(Integer.parseInt(temp[0]), temp[1], temp[3].toLowerCase(), "", canBeBought, Integer.parseInt(temp[7]),
                            new Property(temp[3].toLowerCase(), Integer.parseInt(temp[8]), Integer.parseInt(temp[10]), Integer.parseInt(temp[11]),
